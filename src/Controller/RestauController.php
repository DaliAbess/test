...
class RestauController extends AbstractController
{
     ...
     */
    public function new(Request $request, RestauRepository $restauRepository): Response
    {
        $restau = new Restau();
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            $restauRepository->add($restau, true);
     ...
     */
    public function edit(Request $request, Restau $restau, RestauRepository $restauRepository): Response
    {
        $form = $this->createForm(RestauType::class, $restau);
        $form->handleRequest($request);
     ...
     */
    public function delete(Request $request, Restau $restau, RestauRepository $restauRepository): Response
    {
        if ($this->isCsrfTokenValid('delete'.$restau->getId(), $request->request->get('_token'))) {
            $restauRepository->remove($restau, true);
        }
